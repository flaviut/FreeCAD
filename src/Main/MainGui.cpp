...
int main(int argc, char** argv)
{
    ...
    try {
        ...
        if (startup.shouldExit()) {
            ...
            if (startup.request == App::StartupRequest::VerboseVersion) {
                std::stringstream guiPreferences;
                Gui::ProgramInformation::getStylePreferenceInformation(guiPreferences);
                Gui::ProgramInformation::getNavigationStyleInformation(guiPreferences);
                std::cout << guiPreferences.str();
            }
            exit(0);
        }
#if defined(FC_OS_WIN32)
